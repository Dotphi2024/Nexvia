                ...
                <div class="card border border-light-subtle shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" id="pageTitle" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $page->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="slug" id="pageSlug" class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug', $page->slug) }}" placeholder="e.g. privacy-policy">
                        ...
                        <div class="mb-0">
                            ...
                            <textarea name="content" class="form-control font-monospace" rows="8">{{ old('content', $page->content) }}</textarea>
                        ...
                        <div id="pointsContainer" class="d-flex flex-column gap-3">
                            ...
                            @if(is_array($existingPoints) && count($existingPoints) > 0)
                                @foreach($existingPoints as $index => $point)
                                    <div class="point-row p-3 rounded border border-light-subtle bg-light-subtle position-relative" data-index="{{ $index }}">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <span class="badge bg-primary text-white point-badge">Point #{{ $loop->iteration }}</span>
                                        ...
                                        <div class="mb-2">
                                            <input type="text" name="points[{{ $index }}][title]" class="form-control form-control-sm fw-semibold" value="{{ is_array($point) ? ($point['title'] ?? '') : '' }}" placeholder="Point Title (e.g. Information We Collect)">
                                        </div>
                                        <div>
                                            <textarea name="points[{{ $index }}][description]" class="form-control form-control-sm" rows="2" placeholder="Point Description or details...">{{ is_array($point) ? ($point['description'] ?? '') : (is_string($point) ? $point : '') }}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                ...
                <div class="card border border-light-subtle shadow-sm mb-4">
                    ...
                    <div class="card-body p-3">
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" name="is_active" id="isActiveSwitch" value="1" {{ old('is_active', $page->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="sort_order" class="form-control form-control-sm" value="{{ old('sort_order', $page->sort_order) }}" min="0">
                        ...
                        <div class="p-2 bg-light rounded border border-light-subtle mb-3 small">
                            <div class="d-flex justify-content-between mb-1">
                                ...
                                <span class="fw-semibold text-dark">{{ $page->created_at ? $page->created_at->format('d M Y') : '-' }}</span>
                            </div>
                            <div class="d-flex justify-content-between">
                                <span class="text-muted">Last Updated:</span>
                                <span class="fw-semibold text-dark">{{ $page->updated_at ? $page->updated_at->format('d M Y, h:i A') : '-' }}</span>
                            </div>
                        </div>
                ...
                <div class="card border border-light-subtle shadow-sm mb-4">
                    ...
                    <div class="card-body p-3">
                        <div class="mb-3">
                            ...
                            <input type="text" name="meta_title" class="form-control form-control-sm" value="{{ old('meta_title', $page->meta_title) }}" placeholder="SEO page title">
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold text-dark small">Meta Description</label>
                            <textarea name="meta_description" class="form-control form-control-sm" rows="3" placeholder="Search engine snippet summary...">{{ old('meta_description', $page->meta_description) }}</textarea>
                        </div>

                        <div class="mb-0">
                            <label class="form-label fw-semibold text-dark small">Meta Keywords</label>
                            <input type="text" name="meta_keywords" class="form-control form-control-sm" value="{{ old('meta_keywords', $page->meta_keywords) }}" placeholder="policy, privacy, terms">
                        </div>
                    </div>
                </div>
            </div>
